...
{
	// Static initializers
	LPCWSTR KinectAudioHandler::GrammarFileName = L"SpeechBasics-D2D.grxml";
	...
	int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE /* hPrevInstance */, _In_ LPWSTR /* lpCmdLine */, _In_ int nCmdShow)
	{
		...
		{
			{
				KinectAudioHandler application;
	...
	/// Constructor
	/// </summary>
	KinectAudioHandler::KinectAudioHandler() :
		m_pD2DFactory(NULL),
	...
	/// <param name="hInstance">handle to the application instance</param>
	/// <param name="nCmdShow">whether to display minimized, maximized, or normally</param>
	int KinectAudioHandler::Run(HINSTANCE hInstance, int nCmdShow)
	{
		MSG       msg = { 0 };
		WNDCLASS  wc;
		...
		//	NULL,
		//	(DLGPROC)KinectAudioHandler::MessageRouter,
	...
	LRESULT CALLBACK KinectAudioHandler::MessageRouter(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		KinectAudioHandler* pThis = NULL;

		if (WM_INITDIALOG == uMsg)
		{
			pThis = reinterpret_cast<KinectAudioHandler*>(lParam);
			SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(pThis));
		}
		else
		{
			pThis = reinterpret_cast<KinectAudioHandler*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
		}
	...
	/// <param name="lParam">additional message data</param>
	/// <returns>result of message processing</returns>
	LRESULT CALLBACK KinectAudioHandler::DlgProc(HWND hWnd, UINT message, WPARAM /* wParam */, LPARAM /* lParam */)
	{
		...
		case WM_INITDIALOG:
		{
			...
			/*m_pTurtleController = std::shared_ptr<TurtleController>(new TurtleController());
	...
	HRESULT KinectAudioHandler::StartKinect()
	{
		HRESULT hr = S_OK;
		...
		if (SUCCEEDED(hr))
		{
			...
			m_p16BitAudioStream = std::shared_ptr<KinectAudioStream>(new KinectAudioStream(m_pAudioStream));
		}
	...
	/// </summary>
	/// <returns>S_OK on success, otherwise failure code.</returns>
	HRESULT KinectAudioHandler::InitializeSpeech()
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			hr = m_pSpeechStream->SetBaseStream(m_p16BitAudioStream.get(), SPDFID_WaveFormatEx, &wfxOut);
		}
	...
	/// </returns>
	HRESULT KinectAudioHandler::CreateSpeechRecognizer()
	{
	...
	/// <para>S_OK on success, otherwise failure code.</para>
	/// </returns>
	HRESULT KinectAudioHandler::LoadSpeechGrammar()
	{
	...
	/// <para>S_OK on success, otherwise failure code.</para>
	/// </returns>
	HRESULT KinectAudioHandler::StartSpeechRecognition()
	{
		HRESULT hr = S_OK;
	...
	/// </summary>
	void KinectAudioHandler::ProcessSpeech()
	{
	...
	///// </returns>
	//TurtleAction KinectAudioHandler::MapSpeechTagToAction(LPCWSTR pszSpeechTag)
	//{
	...
	void KinectAudioHandler::SetStatusMessage(const WCHAR* szMessage)
	{
